...
Output: [true3,false,false,false,false]
...
class Solution:
    def kids_with_candies(self, candies: List[int], extra_candies: int) -> List[bool]:
        results = [False for candy in candies]
        if extra_candies < max(candies):
            lower_bound = max(candies) - extra_candies
            i = 0
            while i < len(candies):
                if candies[i] in range(lower_bound, max(candies) + 1):
                    print("in range")
                    print(candies[i])
                    print(f"index: {i}")
                    results[i] = True
            ...
            return [True for candy in candies]
        return results
...
if __name__ == "__main__":
    ...
    candies = [2, 3, 5, 1, 3]
    extra = 3
    print(s.kids_with_candies(candies, extra))
